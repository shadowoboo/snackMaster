...
if($page==0){//第一次進來 要回傳第0~2的評價 以及要產生多少頁碼
    ...
    if($totalRec==0){//商品目前無評價
        ...
        $pageHTML=
        '<ul><li class="page-item"><a  id="last" class="page-link" page=""><i class="fas fa-chevron-left"></i></a></li>';
       for($i=1;$i<$totalPage+1;$i++){
        $pageHTML.='<li class="page-item"><a  class="page-link" page="'.$i.'">'.$i.'</a></li>';
       }
            
        $pageHTML.='<li class="page-item"><a  id="next" class="page-link" page=""><i class="fas fa-chevron-right"></i></a></li>
        </ul>';
        $rsp->pages=$pageHTML;
        ...
        '<div class="mem">
            ...
            <div class="memCol">
                <div class="memId">
                    <p>'.$cmt["memId"].'</p><button class="report" to="cmt" repNo='.$cmt["evaNo"].'>...</button>
                </div>
    ...
    <div class="memCol">
        <div class="memId">
            <p>'.$cmt["memId"].'</p><button class="report" repNo='.$cmt["evaNo"].'>...</button>
        </div>
        <div class="star" grad="'.$cmt["goodStar"].'"><img src="../images/rankBoard/starMask.png" alt="星等"></div>
        <p class="commentCtx">
        '.$cmt["evaCtx"].'
